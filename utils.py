...
def path_planning(num_frames, x, y, z, path_type=''):
    if path_type == 'straight-line':
        ...
        for frame_id, bs_shift_val in enumerate(np.arange(-2.0, 2.0, (4./num_frames))):
            ...
            zs += [np.cos(bs_shift_val * np.pi/2.) * 1 * z]
        xs, ys, zs = np.array(xs), np.array(ys), np.array(zs)
    elif path_type == 'circle-reverse':
        xs, ys, zs = [], [], []
        for frame_id, bs_shift_val in enumerate(np.arange(-2.0, 2.0, (4./num_frames))):
            xs += [np.sin(bs_shift_val * np.pi) * 1 * x]
            ys += [np.cos(bs_shift_val * np.pi) * 1 * y]
            zs += [np.sin(bs_shift_val * np.pi/2.) * 1 * z]
        xs, ys, zs = np.array(xs), np.array(ys), np.array(zs)
    return xs, ys, zs

def add_fade_effect(config, video_path): 
    output_video_name = f"{video_path.split('/')[-1].split('.')[0]}_fade_in_out_effect.mp4"
    output_video_path = os.path.join(config['video_folder'], output_video_name)
    ...
    video.release()
    cmd = f"ffmpeg -i {video_path} -vf 'fade=type=in:duration=1,fade=type=out:duration=1:start_time={seconds-1}' -c:a copy {output_video_path}"
    os.system(cmd)
    return True
