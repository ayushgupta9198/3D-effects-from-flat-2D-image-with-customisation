...
import torch
import os
from utils import path_planning, open_small_mask, clean_far_edge, refine_depth_around_edge, add_fade_effect
...
def output_3d_photo(verts, colors, faces, Height, Width, hFov, vFov, tgt_poses, video_traj_types, ref_pose,
    ...
    for video_pose, video_traj_type in zip(videos_poses, video_traj_types):
        ...
        res_video_path = os.path.join(output_dir, video_basename + '_' + video_traj_type + '.mp4')
        clip.write_videofile( res_video_path, fps=config['fps'])
        if config['use_fade_effect']:
            add_fade_effect(config, res_video_path)
